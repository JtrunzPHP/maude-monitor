...
Usage:
    ...
    python maude_monitor.py --alert-only       # Skip refresh, just check alerts
...
def api_fetch(url, retries=3):
    """Fetch JSON from openFDA with retry logic."""
    import time
...
def fetch_ae_counts(device, start_date, end_date):
    """Fetch adverse event counts by date_received."""
...
def fetch_event_types(device, months_back=3):
    """Fetch event type breakdown for recent period."""
    end = datetime.now()
    start = end - timedelta(days=months_back * 30)
    url = (
        f"https://api.fda.gov/device/event.json"
        f"?search={device['search']}+AND+date_received:[{start.strftime('%Y%m%d')}+TO+{end.strftime('%Y%m%d')}]"
        f"&count=event_type.exact"
    )
    try:
        data = api_fetch(url)
        return {r["term"]: r["count"] for r in (data.get("results", []) if data else [])}
    except Exception:
        return {}


def fetch_recalls(device):
    """Fetch recent recalls for a firm."""
    url = (
...
def refresh_device(device):
    """Fetch data, compute analytics, write CSV. Returns summary dict."""
    ...
    # Stats
    stats_1yr = compute_stats(counts[-12:]) if len(counts) >= 12 else compute_stats(counts)
...
def send_alert_email(summaries):
    """Send alert email for devices that crossed threshold."""
    ...
    if not all([email_to, email_from, smtp_password]):
        print("  Email not configured — printing alerts to console instead:")
        for s in summaries:
            if s and s["alert"]:
                d = s["device"]
                l = s["latest"]
                print(
                    f"    {d['name']} ({d['ticker']}): "
                    f"{l.get('reports', '?')} reports, Z={l.get('z_score', '?')}"
                )
        return
    ...
    body = (
        ...
        f"<p style='color:#6b7280;font-size:13px;'>{datetime.now().strftime('%Y-%m-%d %H:%M')} ET</p>"
    )
    ...
    for s in alerts:
        ...
        z = l.get("z_score", 0)
        color = (
            "#dc2626" if abs(z) >= 3 else "#d97706" if abs(z) >= 2 else "#2563eb"
        )

        body += f"""
        <div style='background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;
                    padding:16px;margin:12px 0;'>
          <h3 style='margin:0 0 8px;'>{d['name']}
            <span style='color:#6b7280;'>({d['ticker']})</span></h3>
          <table style='width:100%;font-size:13px;'>
            <tr><td style='color:#6b7280;padding:4px 8px;'>Latest month</td>
                <td style='font-weight:600;padding:4px 8px;'>
                  {l.get('month','?')}: {l.get('reports',0):,} reports</td></tr>
            <tr><td style='color:#6b7280;padding:4px 8px;'>1yr average</td>
                <td style='padding:4px 8px;'>
                  {s['stats_1yr']['mean']:,}/mo (σ={s['stats_1yr']['sd']})</td></tr>
            <tr><td style='color:#6b7280;padding:4px 8px;'>Z-score</td>
                <td style='font-weight:600;color:{color};padding:4px 8px;'>
                  {"+" if z > 0 else ""}{z}σ</td></tr>
            <tr><td style='color:#6b7280;padding:4px 8px;'>Rate</td>
                <td style='padding:4px 8px;'>
                  {l.get('rate_per_m','N/A')} per $M (avg: {s['rate_avg']})</td></tr>
          </table>
          {"<p style='color:#dc2626;font-size:12px;margin:8px 0 0;'>"
           "Rate elevated 3+ months above average</p>"
           if s['rate_elevated'] else ""}
        </div>"""

    body += (
        "<hr style='border:none;border-top:1px solid #e5e7eb;margin:16px 0;'>"
        f"<p style='font-size:11px;color:#9ca3af;'>"
        f"Threshold: {CONFIG['sigma_threshold']}σ · Data: openFDA MAUDE</p></div>"
    )
    ...
    for s in summaries:
        ...
        <div class="card">
          ...
          <div class="chart-toggle">
            ...
            <button
              onclick="drawChart('{d['id']}','rates',this)">Rate/$M</button>
          </div>
        </div>"""
...
<style>
  * {{ box-sizing: border-box; margin: 0; }}
  ...
  .signal {{ font-size: 11px; font-weight: 600;
  ...
  .chart-toggle button {{ font-size: 11px; padding: 4px 12px;
    border: 1px solid #e5e7eb; border-radius: 6px;
    background: #fff; cursor: pointer; font-family: inherit; }}
  .chart-toggle button.active {{ background: #1a1a1a;
    color: #fff; border-color: #1a1a1a; }}
  .footer {{ max-width: 1000px; margin: 24px auto 0;
...
<div class="header">
  ...
  <p>openFDA adverse events · Updated {now} ·
     Threshold: {CONFIG['sigma_threshold']}σ</p>
</div>
...
function drawChart(deviceId, mode, btn) {{
  ...
  const datasets = [{{
    type: 'bar',
    label: mode === 'rates' ? 'Rate/$M' : 'Reports',
    data: values,
    backgroundColor: mode === 'rates'
      ? 'rgba(249,115,22,0.3)' : 'rgba(59,130,246,0.3)',
    borderColor: mode === 'rates'
...
def main():
    ...
    parser.add_argument("--device", help="Refresh single device by ID (e.g. PODD)")
    parser.add_argument(
        "--alert-only", action="store_true", help="Check alerts without refreshing"
    )
    parser.add_argument(
        "--html", action="store_true", help="Generate HTML dashboard"
    )
    parser.add_argument(
        "--no-email", action="store_true", help="Skip email alerts"
    )
    ...
    for device in devices:
        ...
        try:
            ...
            if summary:
                l = summary["latest"]
                status = "ALERT" if summary["alert"] else "OK"
                print(
                    f"  {'[!]' if summary['alert'] else '[ok]'} {status} — "
                    f"{l.get('month', '?')}: {l.get('reports', 0):,} reports, "
                    f"Z={l.get('z_score', 0)}, "
    ...
    if alerts and not args.no_email:
        print(f"Sending alerts for: {', '.join(s['device']['id'] for s in alerts)}")
